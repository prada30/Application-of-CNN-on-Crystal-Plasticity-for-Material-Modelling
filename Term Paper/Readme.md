...
## Structure
* The Zip file contains 2 folders, 2 python files and 1 markdown file.
* "50 grains" folder contains 245 hdf5 files that are used by the code "data_extraction.py"
* "Data" folder contains 1 h5 file "extracted_data.h5" which is produced by the "data_extraction.py" and used by "model.py"


## Run

* Extract the "Term paper" folder to the desired directory.
* In the "data_extraction.py" file, change line 34 to "path_file = os.path.join(os.getcwd(), "50 grains", "*.hdf5")".
* The "extracted_data.h5" is already provided in "Data" folder. However, running the "data_extraction.py" also produces "extracted_data.h5" file. 
* Ensure "extracted_data.h5" is saved in a folder named "Data".
* In "Model.py", the NN Model layers can be tweaked in the class Custom3dCNN() by addition or removal of layers in both "__init__" and "forward()". Preferrably comment the layers you do not want in the model.
* On running "Model.py", a CNN model is created which is trained on the dataset obtained from "extracted_data.h5". 
* The trained model is saved at the end in a folder named "Model" and the learning curves and metrics are saved in the current directory as images.
